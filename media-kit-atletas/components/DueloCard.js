...
export default function DueloCard({ p1, p2, votes1, votes2, onVote, showVoting = true }) {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto bg-[#111] border border-[#222] rounded-3xl overflow-hidden shadow-2xl relative font-sans">
            {/* BACKGROUND FX */}
            ...
            {/* HEADER */}
            <div className="relative z-10 text-center py-5 bg-[#0c0c0c]/90 border-b border-[#222] backdrop-blur-sm">
                <h2 className="text-2xl md:text-5xl font-black text-white uppercase italic tracking-tighter">
                    TALE OF THE <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-red-600">TAPE</span>
                </h2>
            </div>
            ...
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-3">
                ...
                <div className="order-2 md:order-1 relative flex flex-col items-center p-6 border-r border-[#222]">

                    {/* LINK PARA O PERFIL P1 */}
                    <Link href={`/${p1.slug || p1.id}`} target="_blank" className="group flex flex-col items-center cursor-pointer mb-6">
                        <div className="relative w-32 h-32 md:w-48 md:h-48 rounded-full border-4 border-blue-600 overflow-hidden mb-4 shadow-[0_0_20px_rgba(37,99,235,0.4)] bg-[#1a1a1a] group-hover:border-white group-hover:shadow-[0_0_30px_rgba(255,255,255,0.3)] transition-all duration-300">
                        ...
                        <h3 className="text-2xl md:text-3xl font-black text-white uppercase text-center leading-none mb-2 group-hover:text-blue-400 transition-colors">
                            {p1.apelido || p1.nome}
                        </h3>
                        <p className="text-blue-400 font-bold text-xs uppercase tracking-widest bg-blue-900/20 px-2 py-1 rounded border border-blue-900/30">{p1.categoria}</p>
                    </Link>

                    {showVoting && (
                        <button onClick={() => onVote(1)} className="mt-auto w-full py-4 bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-widest rounded-xl -skew-x-6 transition transform hover:scale-105 shadow-lg shadow-blue-900/50">
                            Votar
                        </button>
                    )}
                </div>
                ...
                {/* STATS (CENTER) */}
                <div className="order-1 md:order-2 flex flex-col justify-center bg-black/20 backdrop-blur-sm">

                    {/* FÍSICO */}
                    <div className="grid grid-cols-3 items-center text-center py-4 border-b border-[#222] hover:bg-[#1a1a1a] transition">
                        <span className={`text-xl font-bold ${cHeight.c1}`}>{p1.stats?.height || '-'}</span>
                        <span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest flex flex-col items-center gap-1"><Ruler size={14} /> Altura</span>
                    ...
                    <div className="grid grid-cols-3 items-center text-center py-4 border-b border-[#222] hover:bg-[#1a1a1a] transition">
                        ...
                        <span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest flex flex-col items-center gap-1"><Scale size={14} /> Envergadura</span>
                        <span className={`text-xl font-bold ${cReach.c2}`}>{p2.stats?.reach || '-'}</span>
                    </div>

                    <div className="grid grid-cols-3 items-center text-center py-4 border-b border-[#222] hover:bg-[#1a1a1a] transition">
                        <span className={`text-xl font-bold ${cAge.c1}`}>{p1.stats?.age || '-'}</span>
                        <span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest flex flex-col items-center gap-1"><User size={14} /> Idade</span>
                        <span className={`text-xl font-bold ${cAge.c2}`}>{p2.stats?.age || '-'}</span>
                    </div>

                    {/* CARTEL */}
                    <div className="grid grid-cols-3 items-center text-center py-4 border-b border-[#222] bg-[#1a1a1a]">
                        <span className={`text-3xl font-black ${cWins.c1}`}>{p1.record?.wins}</span>
                        <span className="text-[10px] text-red-500 uppercase font-bold tracking-widest flex flex-col items-center gap-1"><Trophy size={14} /> Vitórias</span>
                        <span className={`text-3xl font-black ${cWins.c2}`}>{p2.record?.wins}</span>
                    </div>

                    <div className="grid grid-cols-3 items-center text-center py-4 border-b border-[#222] hover:bg-[#1a1a1a] transition">
                        <span className={`text-xl font-bold ${cKo.c1}`}>{p1.record?.knockouts}</span>
                        <span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest flex flex-col items-center gap-1"><Zap size={14} /> Nocautes</span>
                        <span className={`text-xl font-bold ${cKo.c2}`}>{p2.record?.knockouts}</span>
                    </div>

                    {/* REDES SOCIAIS */}
                    <div className="grid grid-cols-3 items-center text-center py-2 border-b border-[#222] hover:bg-pink-900/10 transition">
                    ...
                    <div className="grid grid-cols-3 items-center text-center py-2 border-b border-[#222] hover:bg-cyan-900/10 transition">
                ...
                <div className="order-3 relative flex flex-col items-center p-6 border-l border-[#222]">

                    {/* LINK PARA O PERFIL P2 */}
                    <Link href={`/${p2.slug || p2.id}`} target="_blank" className="group flex flex-col items-center cursor-pointer mb-6">
                        <div className="relative w-32 h-32 md:w-48 md:h-48 rounded-full border-4 border-red-600 overflow-hidden mb-4 shadow-[0_0_20px_rgba(220,38,38,0.4)] bg-[#1a1a1a] group-hover:border-white group-hover:shadow-[0_0_30px_rgba(255,255,255,0.3)] transition-all duration-300">
                        ...
                        <h3 className="text-2xl md:text-3xl font-black text-white uppercase text-center leading-none mb-2 group-hover:text-red-400 transition-colors">
                            {p2.apelido || p2.nome}
                        </h3>
                        <p className="text-red-400 font-bold text-xs uppercase tracking-widest bg-red-900/20 px-2 py-1 rounded border border-red-900/30">{p2.categoria}</p>
                    </Link>

                    {showVoting && (
                        <button onClick={() => onVote(2)} className="mt-auto w-full py-4 bg-red-600 hover:bg-red-500 text-white font-black uppercase tracking-widest rounded-xl -skew-x-6 transition transform hover:scale-105 shadow-lg shadow-red-900/50">
                            Votar
                        </button>
                    )}
                </div>
            ...
            {showVoting && totalVotes > 0 && (
                <div className="relative h-8 bg-[#1a1a1a] w-full flex border-t border-[#333]">
                    ...
                    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-black px-3 py-1 rounded-full border border-[#333]">
