...
import { PlusCircle, Trash2, Lock, Swords } from 'lucide-react';
...
export default function TabLutas({ perfil, setPerfil, handleNextFightChange, isPremium }) {
    ...
    const handleFightChange = (index, field, value) => { 
        const n = [...perfil.historico]; n[index][field] = value; setPerfil({...perfil, historico: n}); 
    };

    const handleDeleteLuta = (index) => {
        const n = [...perfil.historico]; n.splice(index, 1); setPerfil({...perfil, historico: n});
    };

    return (
        <div className="space-y-6">
            {/* PRÓXIMA LUTA (Bloqueado Free) */}
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                <h3 className="text-cyan-400 font-bold uppercase text-sm mb-4 flex gap-2"><Swords size={16}/> Próxima Luta</h3>
                {isPremium ? (
                    ...
                    <div className="bg-slate-950 p-4 rounded border border-dashed border-slate-700 text-center opacity-50"><Lock size={20} className="mx-auto mb-2 text-yellow-500"/><p className="text-xs text-slate-400">Exclusivo Premium</p></div>
                )}
            </div>
            ...
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                <div className="flex justify-between mb-4">
                    ...
                    {(isPremium || perfil.historico.length < 1) && (
                        <button onClick={handleAddLuta} className="text-xs bg-cyan-600 px-3 py-1 rounded flex items-center gap-1"><PlusCircle size={14}/> Adicionar (Vazio)</button>
                    )}
                </div>
                    ...
                    {perfil.historico.map((luta, i) => (
                        <div key={i} className="grid grid-cols-12 gap-2 bg-black/50 p-2 rounded border border-slate-800 items-center">
                            <div className="col-span-2">
                                <select value={luta.result} onChange={(e) => handleFightChange(i, 'result', e.target.value)} className={`w-full p-1 rounded font-bold ${luta.result === 'W' ? 'bg-green-900 text-green-400' : 'bg-red-900 text-red-400'}`}>
                            ...
                            <div className="col-span-1 text-center"><button onClick={() => handleDeleteLuta(i)} className="text-red-500 hover:text-red-400"><Trash2 size={16}/></button></div>
                        </div>
                    ))}
                </div>
            </div>
