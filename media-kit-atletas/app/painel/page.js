...
import { Trash2, PlusCircle, Save, LogOut, Eye, Lock, Instagram, Youtube, Twitter, Camera, Upload, Link as LinkIcon, Check, X, Image as ImageIcon, BarChart3, Users, PieChart, AlertCircle, Building2, Calendar, Medal, Trophy, Share2, Smartphone, Bell, Swords, ExternalLink, Clock, TrendingUp } from 'lucide-react';

// IMPORTAÇÃO DO NOVO COMPONENTE VISUAL
import { AvatarLevel } from '../../components/AvatarLevel';
        ...
        <div className="space-y-6">
            ...
            {activeTab === 'geral' && (
                <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 grid gap-6">
                    ...
                    {/* --- AQUI ESTÁ O NOVO COMPONENTE DE FOTO + MOLDURA --- */}
                    <div className="flex flex-col items-center justify-center p-6 bg-black/40 rounded-xl border border-slate-700 border-dashed">
                        <div onClick={() => openWidget((url) => setPerfil({...perfil, foto_url: url}))} className="cursor-pointer group relative transition-transform hover:scale-105">
                            ...
                            {/* Componente que exibe a moldura e a foto */}
                            <AvatarLevel 
            ...
            {/* ... Restante das abas (Cartel, Lutas, Midia, Metricas, Contato) continuam iguais ... */}
            {activeTab === 'cartel' && (
                <div className="space-y-6">
            ...
            {activeTab === 'metricas' && (
                <div className="space-y-6">
                    <div className="bg-slate-900 p-6 rounded-xl border border-slate-800"><div className="flex items-center gap-2 mb-6"><h3 className="text-cyan-400 font-bold uppercase text-sm flex items-center gap-2"><Eye size={18}/> Visitas no Perfil</h3><div className="ml-auto bg-black px-3 py-1 rounded-full border border-slate-700 text-sm font-mono text-white">Total: {totalViews}</div></div>
