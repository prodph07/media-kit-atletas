...
import { Trash2, PlusCircle, Save, LogOut, Eye, Lock, Instagram, Youtube, Twitter, Camera, Upload, Link as LinkIcon, Check, X, Image as ImageIcon, BarChart3, Users, PieChart, AlertCircle, Building2, Calendar, Medal, Trophy, Share2, Smartphone, Bell, Swords, ExternalLink, Clock } from 'lucide-react';
...
// Função auxiliar para evitar null nos inputs
...
export default function Painel() {
  ...
  useEffect(() => {
    async function getData() {
      ...
      if (data) {
        ...
        // AQUI APLICAMOS safeVal PARA EVITAR O ERRO DE NULL NOS INPUTS
        setPerfil({
            ...data,
            nome: safeVal(data.nome),
            apelido: safeVal(data.apelido),
            categoria: safeVal(data.categoria),
            foto_url: safeVal(data.foto_url),
            about: safeVal(data.sobre), // Mapeando 'sobre' para 'about' e tratando null
            plano: data.plano || 'free',
            tipo_conta: data.tipo_conta || 'atleta',
            template_style: data.template_style || 'padrao',
            slug: safeVal(data.slug),
            stats: { 
                height: safeVal(data.atributos?.height), 
                weight: safeVal(data.atributos?.weight), 
                reach: safeVal(data.atributos?.reach), 
                age: safeVal(data.atributos?.age) 
            },
            record: data.cartel || { wins: 0, losses: 0, draws: 0, knockouts: 0, submissions: 0 },
            contact: { 
                email: safeVal(data.contato?.email), 
                managerEmail: safeVal(data.contato?.managerEmail), 
                phone: safeVal(data.contato?.phone), 
                phoneDisplay: safeVal(data.contato?.phoneDisplay), 
                city: safeVal(data.contato?.city), 
                trainingCenter: safeVal(data.contato?.trainingCenter) 
            },
            nextFight: { 
                date: safeVal(data.prox_luta?.date), 
                event: safeVal(data.prox_luta?.event), 
                opponent: safeVal(data.prox_luta?.opponent), 
                location: safeVal(data.prox_luta?.location) 
            }, 
            socials: { 
                instagram: { 
                    ...instaData, 
                    active: true, 
                    user: safeVal(instaData.user),
                    followers: safeVal(instaData.followers),
                    url: safeVal(instaData.url),
                    stats: {
                        reach: safeVal(instaData.stats?.reach),
                        impressions: safeVal(instaData.stats?.impressions),
                        engagement: safeVal(instaData.stats?.engagement),
                        shares: safeVal(instaData.stats?.shares)
                    },
        ...
        const { data: duelosPendentes } = await supabase
            .from('duelos')
            .select(`id, created_at, desafiante:atletas!atleta_1_id(nome, apelido, foto_url)`)
        ...
        const { data: historico } = await supabase
            .from('duelos')
            .select(`
                id, created_at, status, expires_at, votos_1, votos_2,
                p1:atletas!atleta_1_id(id, nome, apelido, foto_url),
                p2:atletas!atleta_2_id(id, nome, apelido, foto_url)
            `)
            .or(`atleta_1_id.eq.${ATLETA_ID_NUMERICO},atleta_2_id.eq.${ATLETA_ID_NUMERICO}`)
  ...
  const handleDueloAction = async (dueloId, action) => {
    if (action === 'accept') {
        ...
        if(!error) { alert("Duelo Aceito! Agora ele é público."); window.location.reload(); }
  ...
  const handleSave = async () => {
    ...
    // Na hora de salvar, mapeamos 'about' de volta para 'sobre'
    const payload = { 
    ...
    <div className="min-h-screen bg-[#0a0a0c] text-white p-4 pb-32 font-sans">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex overflow-x-auto gap-2 mb-6 pb-2 scrollbar-hide">
            ...
            <button onClick={() => setActiveTab('notificacoes')} className={`relative px-4 py-2 rounded-full text-sm font-bold uppercase flex items-center gap-2 ${activeTab === 'notificacoes' ? 'bg-yellow-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
                <Bell size={16}/> 
            ...
            <button onClick={() => setActiveTab('historico_duelos')} className={`px-4 py-2 rounded-full text-sm font-bold uppercase flex items-center gap-2 ${activeTab === 'historico_duelos' ? 'bg-purple-600' : 'bg-slate-800 text-slate-400'}`}>
                <Swords size={16}/> Meus Duelos
            </button>
        ...
        <div className="space-y-6">
            ...
            {activeTab === 'notificacoes' && (
                <div className="space-y-4">
                    ...
                    {notificacoes.length === 0 ? (
                        <div className="text-center p-10 bg-slate-900 rounded-xl border border-slate-800 text-slate-500">Nenhuma solicitação de duelo pendente.</div>
                    ) : (
            ...
            {activeTab === 'historico_duelos' && (
                <div className="space-y-4">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <Link href="/duelos/criar" className="text-xs bg-yellow-500 text-black px-3 py-1 rounded font-bold flex items-center gap-1"><Swords size={12}/> Criar Novo</Link>
                    </div>
                    ...
                    {meusDuelos.length === 0 ? (
                        ...
                        meusDuelos.map(duelo => {
                            const souP1 = duelo.p1.id === perfil.id;
                            ...
                            return (
                                <div key={duelo.id} className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                                        ...
                                        <div className="text-right"><p className="text-xs text-slate-400 font-bold">{total} Votos</p><p className="text-[10px] text-slate-600">Total</p></div>
            ...
            {activeTab === 'cartel' && (
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                        ...
                        <div className="flex gap-2 mb-4"><input className="w-full bg-black border border-slate-700 p-2 rounded text-white" placeholder="Ex: Campeão 2024" value={novoPremio} onChange={(e) => setNovoPremio(e.target.value)} /><button onClick={handleAddPremio} className="bg-cyan-600 p-2 rounded"><PlusCircle size={20}/></button></div>
                        <ul className="space-y-2">{perfil.premios.map((p, i) => (<li key={i} className="flex justify-between bg-black/50 p-2 rounded border border-slate-800"><span>{p}</span><button onClick={() => {const n = [...perfil.premios]; n.splice(i, 1); setPerfil({...perfil, premios: n})}} className="text-red-500"><Trash2 size={16}/></button></li>))}</ul>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'lutas' && (
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                        <div className="flex justify-between mb-4"><h3 className="text-cyan-400 font-bold uppercase text-sm">Histórico de Lutas</h3><button onClick={handleAddLuta} className="text-xs bg-cyan-600 px-3 py-1 rounded flex items-center gap-1"><PlusCircle size={14}/> Adicionar</button></div>
            ...
            {activeTab === 'midia' && (
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                        <div className="flex justify-between items-center mb-4"><h3 className="text-cyan-400 font-bold uppercase text-sm">Galeria de Fotos</h3><button onClick={() => openWidget((url) => setPerfil({...perfil, galeria: [...perfil.galeria, { full: url, thumb: url }]}), false)} className="bg-cyan-600 px-3 py-1 rounded text-xs flex items-center gap-2"><Upload size={14}/> Upload</button></div>
